...
func (s *Service) ExportSubmissionsAnalysis(examID uint, search, orderBy, orderDir string, moodleSynced *bool, resultType *string) (*bytes.Buffer, error) {
	// 1. Fetch Exam (to get questions and correct options)
	exam, err := s.examRepo.FindExamByID(context.Background(), s.db, examID)
	if err != nil {
		return nil, err
	}
	...
	// Map Question ID -> Question Struct (to access CorrectOption and Name)
	questionsMap := make(map[uint]models.Question)
	// Map Question Index (for chart) -> Question ID
	// We want to order questions by their Name (int) usually, or ID.
	...
	for i, q := range sortedQuestions {
		if !q.IsActive || q.IsCancelled {
			continue // Skip discontinued questions in the graph? Or show them? Let's skip.
		}
		row := i + 2
	...
	if lastDataRow >= 2 {
		// Add Chart
		// Using a simple Column Chart
		// Categories (X axis): Column A (Questions)
		// Values (Y axis): Column B (Failures)
	...
	for i, sub := range submissions {
		...
		colIndex := 10 // Starts after "Tipo" (I is 9th, so 10th is J)
		for _, q := range sortedQuestions {
